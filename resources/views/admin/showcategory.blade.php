...
<table class="table table-hover" style="border:1px" style="text-align: left"> <br><br>
  ...
  <tr style="text-align: center" style="border:1px" style="text-align: left">
    ...
    <td>{{$category -> type}}</td>
    <td>
    <a href="{{url('/admin/deletecategory/' .$category -> id_category)}}" onclick="return confirm('are you sure ?')" style="margin-right: 10px;">
    ...
    <a href="{{url('/admin/editcategory/' .$category -> id_category)}}">
...
<div class="pagination mb-4">
        {{-- previous --}}
        @if ($category_inner->onFirstPage())
            <span>&laquo;</span>
        @else
            <a href="{{ $category_inner->previousPageUrl() }}" rel="prev">&laquo;</a>
        @endif
        {{-- Hiển thị các số trang --}}
        @for ($i = 1; $i <= $category_inner->lastPage(); $i++)
            @if ($i == $category_inner->currentPage())
                <span class="active">{{ $i }}</span>
            @else
                <a href="{{ $category_inner->url($i) }}"
                    class="{{ Request::fullUrlIs(url('/admin/showproduct') . '?page=' . $i . '*') ? 'active' : '' }}" style="margin: 0 5px;font-size: 20px;">{{ $i }}
                  </a>
            @endif
        @endfor
        {{-- next --}}
        @if ($category_inner->hasMorePages())
            <a href="{{ $category_inner->nextPageUrl() }}" rel="next">&raquo;</a>
        @else
            <span>&raquo;</span>
        @endif
    </div>
